...
public class JulSimpleFormmaterParser implements MultiLineLogParser, TableColumnNameSelfDescribable {
  ...
  // 09-Oct-2010 12:46:42
  private SimpleDateFormat[] datePatterns = new SimpleDateFormat[] { new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.ENGLISH),
  ...
  public LogData parse(String event, ParsingContext context) throws ParseException {
    ...
    try {
      if (!tryToFindDate(event)) {
        return null;
      }
      return tryToParseStringBuilder(sb);
  ...
  public boolean tryToFindDate(String s) {
  ...
  private LogData tryToParseStringBuilder(StringBuilder sb) {
  ...
  public LogData parseBuffer(ParsingContext parsingContext) throws ParseException {
    LogData ld = tryToParseStringBuilder(parsingContext.getUnmatchedLog());
    if (ld != null) {
