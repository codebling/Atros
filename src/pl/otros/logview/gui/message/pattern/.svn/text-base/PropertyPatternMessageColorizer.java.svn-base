...
public class PropertyPatternMessageColorizer implements MessageColorizer {
  ...
  // private boolean patternCanonEq = false;
  // private boolean patternCaseInsensitive = false;
  // private boolean patternComments = false;
  // private boolean patternDotAll = false;
  // private boolean patternLiteral = false;
  // private boolean patternMultiline = false;
  // private boolean patternUnicodeCase = false;
  // private boolean patternUnixLines = false;
  private Style style;
  private String name;
  ...
  public void colorize(StyledDocument styledDocument, int offset, int length) throws BadLocationException {
    ...
    for (int i = 0; i <= groupCount; i++) {
      if (StyleProperties.isStyleForGroupDeclared(i, configuration)) {
        style = StyleProperties.getStyle(styledDocument, configuration, "propStyle" + getName(), i);
        MessageColorizerUtils.colorizeRegex(styledDocument, style, text, offset, pattern, i);
      }
    }
  }
